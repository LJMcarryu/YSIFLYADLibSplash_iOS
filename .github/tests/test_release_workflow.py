...
def write_contract(root: Path, state: str) -> None:
    ...
    (root / "Package.swift").write_text(
        ...
        'url: "https://github.com/LJMcarryu/YSIFLYADLibSplash_iOS/'
        f'releases/download/{VERSION}/YSIFLYADLib-SplashOnly.xcframework.zip",\n'
        f'checksum: "{checksum}"\n',
        encoding="utf-8",
    )
    (root / "YSIFLYADLibSplash.podspec").write_text(
        f"s.version = '{VERSION}'\n"
        "s.source = { :http => 'https://github.com/LJMcarryu/"
        f"YSIFLYADLibSplash_iOS/releases/download/{VERSION}/"
        f"YSIFLYADLib-SplashOnly-{VERSION}.zip' }}\n",
        encoding="utf-8",
    )
    ...
    (root / "release-state.json").write_text(json.dumps({
        "schemaVersion": 1, "channel": "ys-splash",
        "repository": "LJMcarryu/YSIFLYADLibSplash_iOS", "version": VERSION,
...
class ReleaseModeContractTests(unittest.TestCase):
    ...
    def test_draft_candidate_requires_trigger_sha_branch_final_formal_and_exact_identity(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ys-mode-formal-candidate-") as directory:
            ...
            for label, overrides in (
                ...
                ("非官方同仓", {"repository": "someone/YSIFLYADLibSplash_iOS"}),
            ):
    ...
    def test_local_contract_rejects_distribution_url_drift(self) -> None:
        for label, filename in (
            ...
            ("CocoaPods", "YSIFLYADLibSplash.podspec"),
        ):
            with self.subTest(label=label), tempfile.TemporaryDirectory(
                ...
                path.write_text(
                    path.read_text(encoding="utf-8").replace(
                        "https://github.com/LJMcarryu/YSIFLYADLibSplash_iOS/",
                        "https://example.invalid/LJMcarryu/YSIFLYADLibSplash_iOS/",
                    ),
                    encoding="utf-8",
                )
                with self.assertRaises(MODE.ContractError):
    ...
    def test_repository_tracks_first_release_lifecycle_without_inventing_history(self) -> None:
        ...
        for name in ("README.md", "CHANGELOG.md", "RELEASING.md"):
            ...
            self.assertNotIn("releases/tag/6.2.2", document)
            self.assertNotIn("releases/tag/6.2.3", document)
...
class ReleaseDownloaderTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.contents = {
            ...
            "YSIFLYADLib-SplashOnly.xcframework.zip": b"spm",
    ...
    def test_formal_release_rejects_untagged_html_url(self) -> None:
        release = release_fixture("formal_release", self.contents)
        release["html_url"] = (
            f"https://github.com/LJMcarryu/YSIFLYADLibSplash_iOS/releases/tag/{DRAFT_SLUG}"
        )
        documented = DOWNLOAD.provenance(
...
class CocoaPodsFixtureTests(unittest.TestCase):
    def test_fixture_rewrites_only_binary_source_and_demo_podspec(self) -> None:
        with tempfile.TemporaryDirectory(prefix="ys-cocoapods-fixture-") as directory:
            ...
            release_dir.mkdir()
            (release_dir / f"YSIFLYADLib-SplashOnly-{VERSION}.zip").write_bytes(b"zip")
            (root / "YSIFLYADLibSplash.podspec").write_text(
                "Pod::Spec.new do |s|\n"
                "  s.name = 'YSIFLYADLib'\n"
                f"  s.version = '{VERSION}'\n"
            ...
            (root / "YSIFLYADLibSimple/Podfile").write_text(
                ...
                "  pod 'YSIFLYADLibSplash', :podspec => 'https://example.invalid/spec'\n"
                "end\n",
                encoding="utf-8",
            )
            ...
            podspec = (output / "YSIFLYADLibSplash.podspec").read_text(encoding="utf-8")
            podfile = (output / "YSIFLYADLibSimple/Podfile").read_text(
                encoding="utf-8"
            )
            self.assertIn(
                (release_dir / f"YSIFLYADLib-SplashOnly-{VERSION}.zip").resolve().as_uri(),
                podspec,
            )
            self.assertIn(hashlib.sha256(b"zip").hexdigest(), podspec)
            self.assertIn(str(output / "YSIFLYADLibSplash.podspec"), podfile)
            self.assertTrue((output / "YSIFLYADLibSimple/project.pbxproj").is_file())
            parsed = json.loads(
                subprocess.check_output(
                    ["pod", "ipc", "spec", str(output / "YSIFLYADLibSplash.podspec")],
                    text=True,
                )
            )
            self.assertEqual(
...
class WorkflowStructureTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        podspec = subprocess.run(
            ["pod", "ipc", "spec", str(ROOT / "YSIFLYADLibSplash.podspec")],
            check=True,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
    ...
    def test_white_labeled_splash_callback_accepts_only_exact_selector(self) -> None:
        checks = load_module("splash_selector_checks", ROOT / "scripts/release_control_plane_checks.py")
        callback = checks.selector_pattern("ysifly_splashAd:didFailWithError:")
        self.assertIn('["ysifly_splashAd:didFailWithError:"]', self.source)
        self.assertIsNotNone(callback.search(
            "- (void)ysifly_splashAd:(YSIFLYSplashAd *)ad\n"
            "    didFailWithError:(YSIFLYAdError *)error;"
        ))
        for declaration in (
            "- (void)splashAd:(YSIFLYSplashAd *)ad didFailWithError:(YSIFLYAdError *)error;",
            "- (void)ysifly_splashAd:(YSIFLYSplashAd *)ad didRejectClickWithError:(YSIFLYAdError *)error;",
            "- (void)ysifly_splashAd:(YSIFLYSplashAd *)ad;",
        ):
    ...
    def test_control_plane_canary_reuses_draft_downloader_without_heavy_builds(self) -> None:
        ...
        self.assertIn("ysifly_splashAd:didFailWithError:", source)
    ...
    def test_release_contracts_accept_only_current_frozen_state(self) -> None:
        ...
        def with_state(value: dict[str, object]):
            def read(root: Path, relative: str) -> str:
                ...
                if relative == "Package.swift":
                    return re.sub(r'checksum:\s*"[^"]+"', 'checksum: "' + "1" * 64 + '"', content)
                if relative in {"README.md", "CHANGELOG.md", "RELEASING.md"}:
                    return content.replace('"releaseState":"PREPARING"', '"releaseState":"FORMAL"')
                return content
    ...
    def test_podspec_comments_cannot_substitute_for_parsed_link_contract(self) -> None:
        podspec_source = (ROOT / "YSIFLYADLibSplash.podspec").read_text(encoding="utf-8")
    ...
    def test_control_scripts_are_pinned_to_workflow_commit_not_release_tag(self) -> None:
        ...
        # 新独立发行仓没有旧 YS 的历史 Tag；控制文件固定性由上面的
        # workflow SHA 和每个消费 job 的 checkout 契约验证。
