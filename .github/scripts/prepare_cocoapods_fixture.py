...
def prepare(root: Path, release_dir: Path, output: Path, version: str) -> None:
    ...
    combined = release_dir / f"YSIFLYADLib-SplashOnly-{version}.zip"
    require(combined.is_file() and not combined.is_symlink(), f"缺少已验证合并 zip: {combined}")
    podspec_source = root / "YSIFLYADLibSplash.podspec"
    demo_source = root / "YSIFLYADLibSimple"
    require(podspec_source.is_file(), "缺少 YSIFLYADLibSplash.podspec")
    ...
    local_podspec = output / "YSIFLYADLibSplash.podspec"
    ...
    replacement = f"  pod 'YSIFLYADLibSplash', :podspec => '{local_podspec}'"
    podfile_text, pod_count = re.subn(
        r"(?m)^\s*pod\s+['\"]YSIFLYADLibSplash['\"]\s*,\s*:podspec\s*=>\s*['\"][^'\"]+['\"]\s*$",
        replacement,
        podfile_text,
    )
